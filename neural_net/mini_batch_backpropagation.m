function [W_deriv, bias_deriv] = mini_batch_backpropagation(net, xbatch, t, derivFunErr)
    
    W_deriv = {};
    deltas = {};
    bias_deriv = {};
    for batch_img = 1 : length(xbatch)
        x = xbatch(:, :, batch_img);
        ...
        %commentare per ric
        delta_out = delta_out .* derivFunErr(z_{end}, t(batch_img, :));
        ...
        for i=net.n_layers-1 : -1: 2
            ...
            if net.layers{i}.type == 2
                ...
                derivs = zeros(size(deltas{i}(:, 1) * z_{end-z}(1, :)));
                W_deriv{i-1} = zeros(size(net.weights{i-1}));

                dim_flatten = size(z_{end-z});
                for k = 1 : dim_flatten(1)
                    derivs = derivs + (deltas{i}(:, k) * z_{end-z}(k, :));
                end
                
                W_deriv{i-1} = W_deriv{i-1} + derivs';
                
            else
                deltas{i} = net.weights{end-w}' * deltas{i+1};
                deltas{i} = deltas{i} .* net.deriv_func{end-a}(a_{end-a});
                
                W_deriv{i-1} = W_deriv{i-1} + deltas{i} * z_{end-z}';
                
                if net.layers{i}.use_bias == 1
                    bias_deriv{i-1} = bias_deriv{i-1} + deltas{i};
                end
            end
            %disp(size(deltas{i}));
    ...
    %media gradiente sui batch
    for i=2 : net.n_layers
       W_deriv{i-1} = W_deriv{i-1} ./ length(xbatch);
       bias_deriv{i-1} = bias_deriv{i-1} ./ length(xbatch);
    end
end