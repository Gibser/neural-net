function net=net(n_neurons, actv_functions, deriv_func, n_layers)
    ...
    net.biasMomentums = {};
    for i=2 : n_layers
        net.weights{i-1} = SIGMA*randn(n_neurons(i), n_neurons(i-1));%gpuArray(SIGMA*randn(n_neurons(i), n_neurons(i-1)));
        net.biases{i-1} = SIGMA*randn(n_neurons(i), 1);%gpuArray(SIGMA*randn(n_neurons(i), 1));
        net.weightMomentums{i-1} = SIGMA*randn(n_neurons(i), n_neurons(i-1)); %momenti per i pesi per l'RPROP
